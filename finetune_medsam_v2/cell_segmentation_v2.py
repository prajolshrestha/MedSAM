...
class CellDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        bboxes[:, [0, 2]] /= 256 #self.image_size # normalize
        bboxes[:, [1, 3]] /= 256 #self.image_size
        ...
        one_hot_mask = np.zeros((len(unique_labels), 256, 256), dtype=np.uint8)
    ...
    def forward(self, image, boxes):
        ...
        for i in range(batch_size):
            ...
            low_res_masks, _ = self.mask_decoder(
                image_embeddings=image_embed.repeat(len(box_batch), 1, 1, 1),
            ...
            outputs.append(ori_res_masks.squeeze(1))  # Shape: (N, 256, 256)
        
        return outputs  # List of tensors, each with shape (N, 256, 256)
...
def main():
    ...
    for epoch in range(num_epochs):
        ...
        for step, (image, mask, bboxes, _) in enumerate(tqdm(train_dataloader)):
            ...
            medsam_preds = medsam_model(image, bboxes)
            
            batch_loss = 0
            for i in range(len(medsam_preds)):
                pred = medsam_preds[i]
                gt = mask[i]
                
                # Adjust predictions to match the number of ground truth instances
                num_instances = gt.shape[0]
                if pred.shape[0] < num_instances:
                    pred = F.pad(pred, (0, 0, 0, 0, 0, num_instances - pred.shape[0]))
                elif pred.shape[0] > num_instances:
                    pred = pred[:num_instances]
                
                # Calculate loss
                batch_loss += seg_loss(pred, gt) + ce_loss(pred, gt.float())
            
            loss = batch_loss / len(medsam_preds)
            loss.backward()
            optimizer.step()
